...
import { Header, Navegacion, FormSalida, FormProductosSalida, FormResumenSalida, FloatingButton } from "../components";

interface Props {
    productos: IProducto[];
}

export default function Salida({ productos }: Props) {

    const [renderFormSalida, setRenderFormSalida] = useState(true);
    ...
    const [salida, setSalida] = useState<ISalida>({} as ISalida);

    const { register, getValues, setValue, handleSubmit } = useForm<ISalida>();

    const renderFormProductosHandler = (e: FormEvent) => {
        e.preventDefault();

        if (getValues(["almacenista", "responsable", "conformidad", "fecha"]).includes("")) {
            return alert("Todos los campos son obligatorios");
        }

        setRenderFormSalida(false);
        setRenderFormProductos(true);
    }

    const onSubmit = handleSubmit( async (data) => {
        try {
            await cecatiAPI.post("/salida", data);
    ...
    return (
        <>
            ...
            <form id="salida" onSubmit={onSubmit} >
                {renderFormSalida &&
                    <FormSalida 
                        register={register}
            ...
            {renderFormResumen && <FormResumenSalida data={salida} /> }
        </>
    )
}
